...
// Module
___CSS_LOADER_EXPORT___.push([module.id, "body{font-weight:bold;line-height:300%;letter-spacing:1px}html,body:not(#range){overflow:hidden;touch-action:pinch-zoom}.canvas-wrap{border-style:solid;border-width:1px}@media(orientation: landscape){.canvas-wrap{width:min(90vw,90vh);height:min(90vw,90vh)}}@media(max-aspect-ratio: 1110/967){.canvas-wrap{width:min(85vw,85vh);height:min(85vw,85vh)}}.canvas{position:absolute}@media(orientation: landscape){.canvas{width:min(90vw,90vh);height:min(90vw,90vh)}}@media(max-aspect-ratio: 1110/967){.canvas{width:min(85vw,85vh);height:min(85vw,85vh)}}#flex1{display:flex;flex-flow:row wrap;align-items:stretch;justify-content:space-evenly}@media(orientation: landscape){#flex1{margin-top:3vh}}@media(max-aspect-ratio: 1110/967){#flex1{margin-top:0}}#flex2{display:flex;flex-flow:row wrap;align-items:center}@media(orientation: landscape){#flex2{justify-content:space-around;width:20vw}}@media(max-aspect-ratio: 1110/967){#flex2{justify-content:space-between;width:min(85vw,85vh)}}.check,.radio{position:absolute !important;clip:rect(1px, 1px, 1px, 1px)}.check-label{padding:.5rem 1rem;border-radius:5px;transition:all .2s}.radio-label{padding:.5rem 1rem;border-radius:5px;transition:all .2s}.check:focus+.check-label{outline-width:5px;outline-style:auto;outline-offset:3px}.radio:focus+.radio-label{outline-width:5px;outline-style:auto;outline-offset:3px}#range-label{margin-bottom:-25px;text-align:center}#range{appearance:none;height:14px;border-radius:5px}@media(orientation: landscape){#range{width:20vw}}@media(max-aspect-ratio: 1110/967){#range{width:min(85vw,85vh)}}#range::-webkit-slider-thumb{width:24px;height:24px;appearance:none;border-style:solid;border-width:3px;border-radius:50%}#range::-moz-range-thumb{width:24px;height:24px;border-style:solid;border-width:3px;border-radius:50%}@media(prefers-color-scheme: light){body{background-color:#ece8ef}.canvas-wrap{border-color:#1d3461}.check-label{color:#dc493a}.radio-label{color:#4392f1}.check:checked+.check-label{color:#ece8ef;background:#dc493a}.check:focus+.check-label{outline-color:#dc493a}.radio:checked+.radio-label{color:#ece8ef;background:#4392f1}.radio:focus+.radio-label{outline-color:#4392f1}#range-label{color:#1d3461}#range{background:#1d3461}#range::-webkit-slider-thumb{background:#ece8ef;border-color:#1d3461}#range::-moz-range-thumb{background:#ece8ef;border-color:#1d3461}}@media(prefers-color-scheme: dark){body{background-color:#2e2836}.canvas-wrap{border-color:#ece2d0}.check-label{color:#ee4266}.radio-label{color:#5cc8ff}.check:checked+.check-label{color:#2e2836;background:#ee4266}.check:focus+.check-label{outline-color:#ee4266}.radio:checked+.radio-label{color:#2e2836;background:#5cc8ff}.radio:focus+.radio-label{outline-color:#5cc8ff}#range-label{color:#ece2d0}#range{background:#ece2d0}#range::-webkit-slider-thumb{background:#2e2836;border-color:#ece2d0}#range::-moz-range-thumb{background:#2e2836;border-color:#ece2d0}}", ""]);
// Exports
...
// EXTERNAL MODULE: ./node_modules/css-loader/dist/cjs.js??ruleSet[1].rules[0].use[1]!./node_modules/sass-loader/dist/cjs.js!./src/index.scss
var cjs_js_src = __webpack_require__(766);
...
;// CONCATENATED MODULE: ./src/models/charge.js
class Charge {
  constructor () {
    this.x = []
    this.y = []
...
;// CONCATENATED MODULE: ./src/models/electric-field.js
class ElectricField {
  constructor (w, h) {
    this.w = w
    this.h = h
...
;// CONCATENATED MODULE: ./src/models/render.js
function tri (ctx) {
  ctx.moveTo(-3, 0)
  ctx.lineTo(3, 0)
  ctx.moveTo(1, 2)
...
function clear (w, h, ctx) {
  ctx.clearRect(0, 0, 2 * w, 2 * h)
}

function Render (w, h, arrayR, arrayTheta, ctx, dark) {
  ctx.lineWidth = 1
  const frequency = 14
...
function RenderCircle (h, c, ctx, dark) {
  for (let i = 0; i < c.l; i++) {
    if (c.q[i] > 0) {
      if (dark === true) {
...
const gpu = new gpu_browser.GPU()
const gpuCanvas = new gpu_browser.GPU({ canvas: canvas })
const width = 300
const height = 300
canvas.width = width
canvas.height = height
...
function canvasClick (a) {
  ...
  if (Number(inputElem.value) !== 0 && src_radio === 'install') {
...
function canvasMousedown (a) {
  ...
  mouseX = canvasX
  mouseY = canvasY
  if (Number(inputElem.value) !== 0 && src_radio === 'move') {
...
function canvasMousemove (a) {
  if (Number(inputElem.value) !== 0 && src_radio === 'move' && c.move.includes(true) === true) {
    const rect = a.target.getBoundingClientRect()
    const viewX = a.clientX - rect.left
    const viewY = a.clientY - rect.top
    const scaleWidth = canvas.clientWidth / width
